...
# Get repeated suits and numbers
def getreps(vin):
...
class gameState:
    ...
    def __init__(self, num, mods):
        ...
        self.str = np.zeros((num, 6), np.float64)  # Hand strengths
    ...
    def gamEval(self):
        for i in range(4):
            ...
            self.cards[:, 4] = (
                lno(cRiver) * self.cards[:, 4] + cRiver * self.deck[:, 19]
            )
        self.showdown()

    # Stage level game loop (Pre-Flop, Flop, Turn, River)
    def action(self):
        ...
        for i in range(6):
            c1 = self.moves[:, i] == 0
            self.moves[:, i] = c1 * self.moves[:, i] + lno(c1) * -1 * np.ones(
                (self.num), np.int64
            )
    ...
    def step(self):
        ...
        addBets = np.resize(
            np.array([dif * 0, dif, dif + 4, dif + 8, dif + 16, dif + 32, dif + 64]),
            (7, self.num),
        )
        addEncode = (movesWide == movesComp) * addBets
        addSqz = np.sum(addEncode, axis=0)
        self.bets[:, self.turn] = self.done[:, 0] * self.bets[:, self.turn] + lno(
            self.done[:, 0]
        ) * (self.bets[:, self.turn] + addSqz)
        self.stacks[:, self.turn] = self.done[:, 0] * self.stacks[:, self.turn] + lno(
            self.done[:, 0]
        ) * (self.stacks[:, self.turn] - addSqz)
        ...
        for i in range(1, 6):
            ...
            self.moves[:, k] = c2 * self.moves[:, k] + lno(c2) * -1 * np.ones(
                (self.num), np.int64
            )

        self.turn = (self.turn + 1) % 6
        self.done[:, 0] = lor.reduce(
            (
                self.done[:, 0],
                self.turn == self.lastBet[:, 0],
                np.sum(self.moves == 0, axis=1) == 5,
            )
        )
        self.ctr += 1
    ...
    def getStensor(self):
        ...
        self.modin = np.concatenate(
            (
                ...
                self.cards % 4 / 4,
                np.concatenate(
                    (self.moves[:, idx:6] / 6, self.moves[:, 0:idx] / 6), axis=1
                ),
                np.concatenate(
                    (self.bets[:, idx:6] / 250, self.bets[:, 0:idx] / 250), axis=1
                ),
                np.concatenate(
                    (self.stacks[:, idx:6] / 2000, self.stacks[:, 0:idx] / 2000), axis=1
                ),
                self.pot / 5000,
            ),
            axis=1,
    ...
    def showdown(self):
        ...
        cToak = lnd.reduce(
            (cToak, lno(cStrf), lno(cFoak), lno(cFull), lno(cFlsh), lno(cStra))
        )
        cTwop = lnd.reduce(
            (
                cTwop,
        ...
        self.str = (
            cHigh[:, :, 4] * 13.0**0 * self.scd["high"][:, :, 4]
            + cHigh[:, :, 3] * 13**1 * self.scd["high"][:, :, 3]
            + cHigh[:, :, 2] * 13**2 * self.scd["high"][:, :, 2]
            + cHigh[:, :, 1] * 13**3 * self.scd["high"][:, :, 1]
            + cHigh[:, :, 0] * 13**4 * self.scd["high"][:, :, 0]
            + cPair * np.float64(13**5) * self.scd["high"][:, :, 2]
            + cPair * np.float64(13**6) * self.scd["high"][:, :, 1]
            + cPair * np.float64(13**7) * self.scd["high"][:, :, 0]
            + cPair * np.float64(13**8) * self.scd["pair"][:, :, 0]
            + cTwop * np.float64(13**9) * self.scd["high"][:, :, 0]
            + cTwop * np.float64(13**10) * self.scd["pair"][:, :, 1]
            + cTwop * np.float64(13**11) * self.scd["pair"][:, :, 0]
            + cToak * np.float64(13**12) * tmp0
            + cToak * np.float64(13**13) * self.scd["toak"][:, :, 0]
            + cStra * np.float64(13**14) * self.scd["stra"]
            + cFlsh * np.float64(13**15) * self.scd["flsh"][:, :, 4]
            + cFlsh * np.float64(13**16) * self.scd["flsh"][:, :, 3]
            + cFlsh * np.float64(13**17) * self.scd["flsh"][:, :, 2]
            + cFlsh * np.float64(13**18) * self.scd["flsh"][:, :, 1]
            + cFlsh * np.float64(13**19) * self.scd["flsh"][:, :, 0]
            + cFull * np.float64(13**20) * self.scd["pair"][:, :, 0]
            + cFull * np.float64(13**21) * self.scd["toak"][:, :, 0]
            + cFoak * np.float64(13**22) * tmp2
            + cFoak * np.float64(13**23) * self.scd["foak"]
            + cStrf * np.float64(13**24) * self.scd["strf"]
        )

        for i in range(6):
            c0 = self.moves[:, i] == np.zeros((self.num), np.int32)
            self.str[:, i] = (
                c0 * np.zeros((self.num), np.int32) + lno(c0) * self.str[:, i]
            )

        winctr = np.zeros(self.num, np.int32)
        ...
        for i in range(6):
            ...
            self.stacks[:, i] = lno(c2) * self.stacks[:, i] + c2 * (
                self.stacks[:, i] + amt
            )
        ...
        if self.dbsd:
            ...
            cFoak = self.scd["foak"] != -1
            cFull = lnd(
                self.scd["toak"][:, :, 0] != -1, self.scd["pair"][:, :, 0] != -1
            )
            cFlsh = self.scd["flsh"][:, :, 0] != -1
            cStra = self.scd["stra"] != -1
            cToak = self.scd["toak"][:, :, 0] != -1
            cTwop = lnd(
                self.scd["pair"][:, :, 0] != -1, self.scd["pair"][:, :, 1] != -1
            )
            cPair = self.scd["pair"][:, :, 0] != -1
            cHigh = self.scd["high"] != -1
    ...
    def getScd(self):
        ...
        sev[:, :, :2] = self.hands
        sev[:, :, 2:5] = np.resize(
            np.repeat(self.deck[:, 13:16], 6, axis=0), (self.num, 6, 3)
        )
        ...
        for i in range(7):
            ...
            for j in range(4):
                scd["flsh"][:, :, 4 - j] = (
                    lno(c0) * scd["flsh"][:, :, 4 - j] + c0 * scd["flsh"][:, :, 3 - j]
                )
            scd["flsh"][:, :, 0] = lno(c0) * scd["flsh"][:, :, 0] + c0 * nos[:, :, i]

            c0 = lnd(cToak, scd["toak"][:, :, 0] < nos[:, :, i])
            scd["toak"][:, :, 1] = (
                lno(c0) * scd["toak"][:, :, 1] + c0 * scd["toak"][:, :, 0]
            )
            ...
            for j in range(2):
                scd["pair"][:, :, 2 - j] = (
                    lno(c0) * scd["pair"][:, :, 2 - j] + c0 * scd["pair"][:, :, 1 - j]
                )
            ...
            for j in range(4):
                scd["high"][:, :, 4 - j] = (
                    lno(c0) * scd["high"][:, :, 4 - j] + c0 * scd["high"][:, :, 3 - j]
                )
        ...
        cAce = nos[:, :, -1] == 12
        tmp = lno(cAce) * -10 * np.ones((self.num, 6), np.int64) + cAce * np.zeros(
            (self.num, 6), np.int64
        )
        nos += 1
...
def make_machine(machine):
    ...
    neurons = np.linspace(
        first_layer_neurons, last_layer_neurons, num_layers, dtype=np.int32
    )
    ...
    for i in range(len(neurons)):
        ...
        if ctr % dropout_period == 0:
            layers.append(keras.layers.Dropout(dropout_frequency))
        ctr += 1
...
# Make models play against themselves for data
...
def play_games(num, mod_pks, mod_names, game_pk):
    modsin = []
    for pk in mod_pks:
        modsin.append(keras.models.load_model(f"machines/{pk}.h5"))
    mods = []
    for i in range(6):
        idx = i % len(modsin)
        mods.append(modsin[idx])
        idx += 1
    gs = gameState(min(num, 100000), mods)
    t0 = time.perf_counter()
    gs.gamEval()
    t1 = time.perf_counter()
    ...
    pyplot.savefig(f"lifter-out/{game_pk}.png")
